...
public sealed class ConservationGameCamera
{
    private bool _panning;
    private bool _moved;
    private readonly IInputManager _inputManager;

    public ConservationGameCamera(IInputManager inputManager)
    {
    ...
    public void Update(float delta)
    {
        ...
        if (_inputManager.HandleActionIfInvoked(Constants.Action_Click_Start))
        {
            _panning = true;
            _moved = false;
        }
        else if (_panning && _inputManager.IsActionInvoked(Constants.Action_Pan))
        {
            ...
            {
                _moved = true;
                _inputManager.MarkActionAsHandled(Constants.Action_Pan);
                Target -= mouseDelta / Zoom;
            }
        }

        if (_panning && _moved && _inputManager.HandleActionIfInvoked(Constants.Action_Click))
        {
            _panning = false;
        }
    }
